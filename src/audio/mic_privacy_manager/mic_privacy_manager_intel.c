...
{
	LOG_DBG("mic_privacy handle_dmic_irq");
...
{
	LOG_DBG("mic_privacy handle_fw_managed_irq");
...
int mic_privacy_manager_init(void)
{
	...
	if (mic_privacy_policy == MIC_PRIVACY_FW_MANAGED) {
		LOG_INF("mic_privacy init FW_MANAGED");
...
{
	LOG_DBG("mic_privacy_manager_get_policy ");
				   ...
				   uint32_t mic_disable_status)
{
	if (mic_privacy_policy == MIC_PRIVACY_HW_MANAGED) {
		...
		if (mic_disable_status != 0) {
			LOG_DBG("mic_privacy_set_gtw_mic_state MUTED");
			mic_priv_data->mic_privacy_state = MIC_PRIV_MUTED;
			mic_priv_data->dma_data_zeroing = true;
			mic_privacy_api->set_fw_mic_disable_status(true);
		} else {
			LOG_DBG("mic_privacy_set_gtw_mic_state UNMUTED");
			mic_priv_data->mic_privacy_state = MIC_PRIV_UNMUTED;
			 ...
			 struct comp_buffer *buffer, uint32_t copy_samples)
{
	...
	if (mic_priv->mic_privacy_state == MIC_PRIV_FADE_IN) {
	...
	} else if (mic_priv->mic_privacy_state == MIC_PRIV_MUTED) {
		buffer_zero(buffer);
	} else {
		LOG_ERR("mic_privacy_process invalid state %x", mic_priv->mic_privacy_state);
	}
}
